...
List<User> userSearch({String? query, List<User>? allUsersList}){
  ...
  allUsersList!.forEach((user) {
    if(user.firstName!.toLowerCase().contains(query!.toLowerCase()) 
        ...
        || '${user.firstName![0] + user.lastName![0]}'.toLowerCase().contains(query.toLowerCase())
    ){
...
List<int> userSearchWithIndex({String? query, List<User>? allUsersList}){
  ...
  for(int i = 0 ; i < length ; i++) {
    ...
    if(allUsersList[i].firstName!.toLowerCase().contains(query!.toLowerCase())
        || allUsersList[i].lastName!.toLowerCase().contains(query.toLowerCase())
        || '${allUsersList[i].firstName! + " " + allUsersList[i].lastName!}'.toLowerCase().contains(query.toLowerCase())
        || allUsersList[i].phone!.toLowerCase() == query.toLowerCase()
        || '${allUsersList[i].firstName![0] + allUsersList[i].lastName![0]}'.toLowerCase().contains(query.toLowerCase())
    ){
