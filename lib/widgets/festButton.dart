...
Widget FestButton(
    {Event? event, List<Event>? individualEventsList, BuildContext? context}) {
  Event? myChildEvent;
  if(event!.isEvent){
    ...
    child: Column(
      children: [
        GestureDetector(
          onTap: () {
            if(event.isEvent){
              Navigator.push(context!, MaterialPageRoute(builder: (context) => EventPage(event: myChildEvent!)));
            } else{
              Navigator.push(context!, MaterialPageRoute(builder: (context) => FestPage(event: event,)));
            }
          },
          child: Container(
