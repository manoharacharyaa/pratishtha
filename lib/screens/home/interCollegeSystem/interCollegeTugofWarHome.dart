...
class InterCollegeTugOfWarHome extends StatefulWidget {
  ...
  State<InterCollegeTugOfWarHome> createState() =>
      _InterCollegeTugOfWarHomeState();
}

class _InterCollegeTugOfWarHomeState extends State<InterCollegeTugOfWarHome> {
  late Future<List<InterCollegeTugOfWarMatch>> allTugOfWarMatches;
  bool isExtended = false;

  @override
  void initState() {
    super.initState();
    allTugOfWarMatches = InterCollegeServices()
        .getAllTugOfWarMatches(widget.currentAcademicYear);
    print(allTugOfWarMatches);
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Column(
        children: [
          FutureBuilder<List<InterCollegeTugOfWarMatch>>(
            future: allTugOfWarMatches,
            builder: (context, snapshot) {
              ...
              if (snapshot.hasError) {
                ...
                return Center(
                    child: Text(
                        'Error in loading TugOfWar Matches, Try after some time'));
              }

              if (!snapshot.hasData || snapshot.data!.isEmpty) {
                print("No TugOfWar Matches Available");
                return Center(child: Text('No TugOfWar Matches Available'));
              }

              List<InterCollegeTugOfWarMatch> matches = snapshot.data!;
